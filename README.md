...
This project investigates the thermodynamic limits of effective resistance on RGGs as the number of nodes $n \to \infty$, with a focus on two connectivity regimes: the **supercritical regime**, in which the mean degree $k$ exceeds the percolation threshold (approximately 4.51, per Quintanilla et al. (2000) and Penrose (2003)), and the **connected regime**, in which $k$ scales with $ln(n)$ to guarantee full connectivity almost surely per Penrose (2003). 
